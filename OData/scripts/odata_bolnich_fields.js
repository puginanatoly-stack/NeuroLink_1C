...
(async () => {
  const d = await get(`${base}Document_БольничныйЛист?$format=json&$top=1`);
  const r0 = (d.value || [])[0] || {};
  console.log("=== Поля Document_БольничныйЛист ===");
  Object.keys(r0).forEach((k) => console.log("  " + k + " = " + JSON.stringify(r0[k]).slice(0, 160)));
