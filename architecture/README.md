# Архитектура моста Tuning 1C (прототип)
...
МСП-серверы (НАБОР ЕЩЕ НЕ ОПРЕДЕЛЕН. Т.К. ЭТО КОНЦЕПЦИЯ): `1c-odata` — OData-доступ к 1С; `memory-mcp` — память/контекст-сервер, И ДР
вероятный кандидат на retrieval-слой. Векторное хранилище — корпоративный эмбендинг.  // Qdrant: embedding episodic- и semantic-памяти
